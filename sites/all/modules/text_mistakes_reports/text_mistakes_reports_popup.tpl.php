<div id="mistakes_popup_panel">
    <div class="popup_backgroung" id="mistakes_popup_background">
        <div class="popup_content" id="mistakes_popup_content">
            <h3 id="popup_title">
              <?php print t('Popup Title'); ?>
            </h3>
            <div class="popup_text_container">
                <span class="popup_text_container_title"><strong><?php print t('Selected Text:'); ?></strong></span>
                <div class="mistakes_popup_text">
                  <?php print t('Selected Text'); ?>
                </div>
            </div>
            <div class="popup_button_panel">
                <button id="popup_send" type="button" class="btn btn-dark">
                  <?php print t('Send'); ?>
                </button>
                <button id="popup_hide" type="button" class="btn btn-dark">
                  <?php print t('Cancel'); ?>
                </button>
            </div>
        </div>
    </div>
</div>
<div class="popup_info" id="mistakes_popup_info">
    <strong id="popup_info_text">
      <?php print t('Popup Info'); ?>
    </strong>
</div>
